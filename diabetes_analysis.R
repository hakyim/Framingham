...
##============================ Read In Files ==================================##
## Consent Group 1
data_age1<-read.delim("phs000007.v23.pht003099.v2.p8.c1.vr_dates_2011_m_0689s.HMB-IRB-MDS.txt.gz", header=T, as.is=T, skip=10, sep='\t')
data_orig1<-read.delim("phs000007.v23.pht000040.v4.p8.c1.dbt0_27s.HMB-IRB-MDS.txt.gz", header=T, as.is=T, skip=10, sep='\t')
data_offs1<-read.delim("phs000007.v23.pht000041.v5.p8.c1.dbt1_7s.HMB-IRB-MDS.txt.gz", header=T, as.is=T, skip=10, sep='\t')

## Consent Group 2
data_age2<-read.delim("phs000007.v24.pht003099.v3.p9.c2.vr_dates_2011_m_0689s.HMB-IRB-NPU-MDS.txt.gz", header=T, as.is=T, skip=10, sep='\t')
data_orig2<-read.delim("phs000007.v24.pht000040.v4.p9.c2.dbt0_27s.HMB-IRB-NPU-MDS.txt.gz", header=T, as.is=T, skip=10, sep='\t')
data_offs2<-read.delim("phs000007.v24.pht000041.v6.p9.c2.dbt1_7s.HMB-IRB-NPU-MDS.txt.gz", header=T, as.is=T, skip=10, sep='\t')

##============================ Organize Data ==================================##
## org_data: data_orig + data_offs + data_age -> data_result
## Organizes the data into the format desired given diabetes data for original cohort, offspring cohort, and ages/dates
org_data = function(data_orig,data_offs,data_age){
...
data_result1 <- org_data(data_orig1, data_offs1, data_age1)      ##Consent Group 1
data_result2 <- org_data(data_orig2, data_offs2, data_age2)      ##Consent Group 2

## We proceed with data for Consent Group 1
